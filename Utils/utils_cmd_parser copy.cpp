...
static std::vector<ParsedToken> parse_json_likeX(const std::string &line) {
    ...
    if (!arg_raw.empty() && arg_raw.front() == '[') {
        ...
        for (auto &kv : kvs) {
            // Array simple: ["arg1","arg2"]
            if (kv.find(':') == std::string::npos) {
                std::string v = remove_outer_quotesX(kv);
                out.push_back({comando, v});
                continue;
            }

            // Array de diccionarios (no lo usás hoy, pero queda soportado)
            size_t pos = kv.find(':');
    ...
    // Caso diccionario: {"server":"x","chunksize":1024}
    if (!arg_raw.empty() && arg_raw.front() == '{') {
        std::string inner = trimX(arg_raw.substr(1, arg_raw.size() - 2));
...
// ============================================================
// Split tokens (CORREGIDO: respeta comillas)
// ============================================================
...
static std::vector<std::string> split_tokensX(const std::string &line, char sep1) {
    ...
    for (char c : line) {
        ...
        // NO cortar por sep1 si estamos dentro de comillas
...
std::vector<ParsedToken> parse_lineX(const std::string &line) {
    ...
    for (auto &t : tokens) {
        ...
        if (pos == std::string::npos) {
            result.push_back({remove_outer_quotesX(t), ""});
            continue;
        }
